...
function reviewRequestKey(taskId,kind){
  return String(taskId||"")+"|"+String(kind||"");
}

function startReviewTimer(taskId,kind){
  reviewRequestStartedAt[reviewRequestKey(taskId,kind)]=performance.now();
}

function reviewElapsedMs(taskId,kind){
  const key=reviewRequestKey(taskId,kind);
...
function requestReviewData(taskId,kind){
  startReviewTimer(taskId,kind);
...
function openTaskReview(taskId){
  ...
  if(cached){
    ...
    document.getElementById("segment-list").innerHTML='<div class="empty">正在讀取 Google Drive 的中文校稿資料…</div>';
    setZhReviewLoadState("正在讀取雲端","working");
  }

  showView("review");
  const sent=requestReviewData(task.id,"zh");
  if(!sent){
...
function renderSegments(items){
  ...
  document.getElementById("stat-uncertain").textContent=
    items.filter(x=>(x.flags||[]).includes("uncertain")).length;
  ...
  document.querySelectorAll(".zh-review-row .confirm").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const row=btn.closest(".zh-review-row");
...
document.getElementById("finalize-zh").addEventListener("click",()=>{
  ...
  const segments=[...document.querySelectorAll("#segment-list .zh-review-row")].map(row=>({
    id:Number(row.dataset.id),
    start:Number(row.dataset.start||0),
    end:Number(row.dataset.end||0),
    text:String(row.querySelector(".zh-polished-final")?.value||"").trim()
  }));
...
window.addEventListener("message",event=>{
  ...
  if(data.type==="review_data"){
    const browserElapsed=reviewElapsedMs(data.task_id,data.kind);
    if(!data.ok){
      if(data.kind==="zh"){
        setZhFinalizeEnabled(false);
      ...
      setZhReviewLoadState(
        shownMs ? "雲端最新・"+(shownMs/1000).toFixed(1)+"s" : "雲端最新",
        "ok"
      );
