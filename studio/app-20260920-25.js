...
function remoteStageStatus(task, stageKey){
  ...
  if(task.remoteStages[stageKey]) return task.remoteStages[stageKey];
  ...
  if(stageKey==="zh"){
    ...
    for(const key of ["polish","asr","metadata"]){
      const item=stages[key];
      if(item && ["error","running","queued"].includes(item.status)){
        return {...item,stage:"zh"};
      }
    }
  }
...
function confirmNextStage(taskId){
  ...
  const retry=remote && remote.status==="error";
...
window.addEventListener("message",event=>{
  ...
  if(data.type==="status_health"){
    ...
    if(data.ok){
      ...
      if(cookieState){
        if(data.youtube_cookies_configured){
