...
Studio 需依「語言設定」工作表動態顯示所有已啟用語言；未來新增語言時，不應修改前端流程名稱。

每個目標語言可分別設定：

- 翻譯文稿：需要 / 不需要
- 文稿來源：AI 翻譯 / 真人翻譯（人工提供）
- 音檔：需要 / 不需要
- 音檔來源：AI TTS / 真人錄音

規則：
- 未勾選的語言不得啟動 Kaggle。
- 文稿來源=真人時，不執行該語言 AI 翻譯；等待人工文稿匯入並確認。
- 音檔來源=真人時，不執行 TTS；等待真人錄音匯入並確認。
- 音檔來源=AI TTS 時，必須先有該語言的正式文稿。
- AI TTS 可以使用 AI 定稿文稿或真人翻譯文稿。
- 各語言互相獨立；某語言失敗不得讓其他已完成語言失效。
- 「各國語言翻譯」Stage 只有在本堂課所有已選擇的文稿需求完成後才標記 done。
- 「各國語言音檔」Stage 只有在本堂課所有已選擇的音檔需求完成後才標記 done。
- 沒有選擇任何其他語言時，不自動執行任何多語翻譯或 TTS。

中央控制表新增「語言任務設定」，每列至少包含：
- task_id
- language_code
- language_name
- transcript_enabled
